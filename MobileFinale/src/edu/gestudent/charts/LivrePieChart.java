 ...
 */
public class LivrePieChart extends AbstractDemoChart {
    ...
    protected CategorySeries buildCategoryDataset(String title, double[] values) {
        ...
        for (Emprunt e : ServicesEmprunt.getInstance().getAllStat()) {

            series.add(e.getName(), e.getNombre());
    ...
    public Form execute() {
        ...
        Integer i = 0;
        for (Emprunt c : ServicesEmprunt.getInstance().getAllStat()) {
            values[i] = (double) c.getNombre();
            i++;
        }
        Integer j = 0;
        for (Emprunt c : ServicesEmprunt.getInstance().getAllStat()) {
